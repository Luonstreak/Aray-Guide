export default function Switch({ label, onChange, active, disabled }){
    return (
        <div className="flex items-center justify-center" onClick={() => !disabled && onChange()}>
            ...
            <label className="relative w-12 h-6 flex select-none cursor-pointer" htmlFor="toggle">
                <span className={`${disabled && 'opacity-50'} absolute left-0 top-0 h-full w-full rounded-full border-2 border-gray-100 ${active ? `bg-primarylight ${!disabled && 'hover:bg-primary'}` : 'bg-gray-200 hover:bg-gray-300'}`}></span>
